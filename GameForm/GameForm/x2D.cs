...
namespace GameForm
{
    public class x2D : Game
    {
        ...
        private Texture2D MySprite,SpriteStone;
        ...
        private bool _press;
        public double xx,yy;
        int[,] _stonemass = new int[19,19]; // массив координат для камней и их цвета
        public int _xstone, _ystone; //x,y координаты камней 
        ...
        protected override void LoadContent()
        {
            ...
            SpriteStone = Content.Load<Texture2D>("stone");
        ...
        protected override void Update(GameTime p_gameTime)
        {
            ...
            if (mouse.LeftButton == ButtonState.Pressed)
            {
                ...
                xx = ((double)(mouse.X - 15)/30);
                
                _xstone = (int)(Math.Round(xx) * 30);
                
                yy = ((double)(mouse.Y - 15) / 30);
                _ystone = (int)(Math.Round(yy) * 30);
                _stonemass[(int)(Math.Round(xx)),(int)(Math.Round(yy))] = 1;
            }
          ...
          //  Vector3 vecScale = new Vector3(2, 2, 2);
           ...
           // _spriteBatch.Draw(MySprite, new Rectangle(20, 20, 536,351), Color.White);
            _spriteBatch.Draw(MySprite, new Rectangle(0,0, sizeSpriteNew, sizeSpriteNew), Color.White);
                 int i,k=0,l=0;
                 for (i = 15; k <= 18; i = i + 30)
                 {
                     _prime2d.DrawLine(i, 15, i, 555, Color.Black);
                     _prime2d.DrawLine(15, i, 555, i,Color.Black);
                     k++;
                 }
                 //if (_press == true)
                 k = 0; l = 0;
                 foreach (int ii in _stonemass)
                 
                 {
                     if (_stonemass[k,l]==1){
                     _spriteBatch.Draw(SpriteStone, new Rectangle(k*30, l*30, 30, 30), Color.White);}
                     if (k < 18)
                     {
                         k++; l++;
                     }
              ...
              //   _spriteBatch.Draw(SpriteStone, new Rectangle(60, 30, 30, 30), Color.White);
               //  _spriteBatch.Draw(SpriteStone, new Rectangle(60, 60, 30, 30), Color.White);
                // _spriteBatch.Draw(SpriteStone, new Rectangle(30, 60, 30, 30), Color.White);
